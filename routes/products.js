...
router.get('/:id', async (req, res) => {
	...
	router.post('/', async (req, res) => {
		try {
			const { name, sku, category, price, quantity, minQuantity, description } =
				req.body
			...
			if (productExists) {
				return res
					.status(400)
					.json({ message: 'Bu SKU bilan tovar allaqachon mavjud' })
			}
...
router.put('/:id', async (req, res) => {
	try {
		const { name, category, price, quantity, minQuantity, description } =
			req.body
